        ...
        <div class="side_left">
            <dl class="side_list">
                ...
                <c:forEach items="${categoryNames}" var="category">
                    <dd class=""><a href="${pageContext.request.contextPath}/phone/brand?method=${category.categoryId}" title="手机回收价格评估"><b
                            class="item${category.categoryId}">&nbsp;</b><em>${category.categoryName}</em><span>&nbsp;</span></a></dd>
                </c:forEach>
            </dl>
        ...
        <div class="side_right">
                ...
                <div class="brand_main">
                    ...
                    <div class="brand_hot">
                            <c:forEach items="${brandNames}" var="brand">
                                <a href="${pageContext.request.contextPath}/phone/brand?brandId=${brand.brandId}">${brand.brandName}</a><em>|</em>
                            </c:forEach>
                    </div>
            ...
            <div class="search_main">
                <div class="search_title"><h2 class="list-item">手机回收</h2>&nbsp;&nbsp;&nbsp;&nbsp;<em></em>搜索到<em>
                    ${phoneList.total} </em>个结果
                </div>
                <dl class="pic_list">
                    <c:forEach items="${phoneList.list}" var="phone">
                    <dd>
                            <div class="pic_list_box">
                                <div class="pic_list_img"><a href="${pageContext.request.contextPath}/phone/info?phoneId=${phone.phoneId}" title="${phone.phoneName}回收价格评估"><img
                                        src="${pageContext.request.contextPath}/${phone.phoneImg}" width="180"
                                        height="135" alt="${phone.phoneName}闲置回收价格评估"/></a></div>
                                <p class="name">${phone.phoneName}</p>
                                <!--<p class="price">回收价：<em>￥3080</em></p>-->
                                <p class="pinggu"><a href="" title="在线评估${phone.phoneName}回收价格"><!--去评估--></a>
                                </p>
                                <div class="hot"><img src="${pageContext.request.contextPath}/images/hot.png"/></div>
                            </div>
                    </dd>
                    </c:forEach>
                ...
                <div class="pageStr">
                    ...
                    <%
                        String abc = null;
                    %>
<%--                        ${param.id}的使用： 若有a.jsp页面跳转到b.jsp后携带参数id过去，在b页面可以使用${param.id}取得该id。适用于查询后刷新原来的页面。--%>
                    <c:if test="${param.brandId!=null}">
                        <%
                            abc = "brandId" +"=" + request.getParameter("brandId");
                        %>
                    </c:if>

                    <c:if test="${phoneList.pageNum != 1}">
                        <span class="last_link"><a href="${pageContext.request.contextPath}/phone/brand?currentPageStr=${phoneList.navigateFirstPage}&<%=abc%>">首页</a></span>
                    ...
                    <c:if test="${phoneList.pageNum != phoneList.pages}">
                        <span class="next_link"><a href="${pageContext.request.contextPath}/phone/brand?currentPageStr=${phoneList.nextPage}&<%=abc%>">下一页</a>
                        <span class="last_link"><a href="${pageContext.request.contextPath}/phone/brand?currentPageStr=${phoneList.navigateLastPage}&<%=abc%>">尾页</a></span>
                    </c:if>
                    </div>
    ...
    <div class="page2">
        ...
        <dl class="foot_list3">
            ...
            <dd>
                ...
                <div><a href="html/hzsr.html" rel="nofollow" target="_blank" class="rk1" title="手机回收业务大客户登录入口">大客户入口</a><a
